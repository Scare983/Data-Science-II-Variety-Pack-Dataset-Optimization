...
def tryPerceptron():
    ...
    yVal1Param = {'model': modelsToUse[0], 'optName': "SGD", 'optimizer': optimizerDict['SGD'], 'activation': activationArray[4], 'epochs':200, 'xVals': xVals.loc[:, [*colsToUse]], "yVals":yVal1, 'num':0}
    for myParams in [yVal1Param]:
        ...
        plt.ylabel('loss')
        plt.legend()
        plt.savefig("./Boston/BestResults/Images/{}{}{}{}".format(myParams['optName'], myParams['model'],myParams['activation'], myParams['num']))
        plt.clf()
...
def tryNeural3L():
    ...
    optimizerDict = {"Adam": optimizers.adam(lr=learningRate), "SGD" : optimizers.sgd(lr=learningRate, clipnorm=.5), "SGD_Moment": optimizers.sgd(lr=learningRate, momentum=.5) }
    yVal1 = allYVals
    colsToUse = [0, 13, 6, 8, 1, 5, 3, 14]

    yVal1Param = {'model': modelsToUse[1], 'optName': "SGD", 'optimizer': optimizerDict['SGD'], 'activation': activationArray[3], 'epochs':200, 'xVals': xVals.loc[:, [*colsToUse]], "yVals":yVal1, 'num':0}
    for myParams in [yVal1Param]:
        myXLength = len(myParams['xVals'].columns)
        myModel =  GeneralModel.buildModel(myParams['model'], myXLength, myParams['activation'], myParams['optimizer'])
        myHistory = myModel.fit(myParams['xVals'], myParams['yVals'], epochs=200, validation_split=.2, verbose=0)
...
def tryNeuralXL():
    ...
    optimizerDict = {"Adam": optimizers.adam(lr=learningRate), "SGD" : optimizers.sgd(lr=learningRate, clipnorm=.5), "SGD_Moment": optimizers.sgd(lr=learningRate, momentum=.5) }
    yVal1 = allYVals
    colsToUse = [0, 13, 6, 5, 8, 1, 11]

    yVal1Param = {'model': modelsToUse[2], 'optName': "SGD", 'optimizer': optimizerDict['SGD'], 'activation': activationArray[3], 'epochs':200, 'xVals': xVals.loc[:, [*colsToUse]], "yVals":yVal1, 'num':0}
    for myParams in [yVal1Param]:
        myXLength = len(myParams['xVals'].columns)
        myModel =  GeneralModel.buildModel(myParams['model'], myXLength, myParams['activation'], myParams['optimizer'])
